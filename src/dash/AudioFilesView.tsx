...
export function AudioFilesView({
  ...
  /** Mirrors `files`, so a slow first load can tell what has been added since it began. */
  const filesRef = useRef<AudioFile[]>([]);

  /** The one way `files` changes, so the library and the ref never drift apart. */
  const showFiles = (next: AudioFile[]) => {
  ...
  useEffect(() => {
    ...
    listAudioFiles()
      .then(loaded => {
        if (cancelled) return;
        // A file added while this was still loading is not in `loaded` — keep it.
        const alreadyLoaded = new Set(loaded.map(file => file.id));
        showFiles([...loaded, ...filesRef.current.filter(file => !alreadyLoaded.has(file.id))]);
      })
      .catch((err: unknown) => {
  ...
  const handleAdded = (added: AudioFile) => {
    showFiles([...filesRef.current, added]);
    setScreen("confirm");
  };
